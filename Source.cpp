...
const int maxstring = 4; // размер в строке значений
const int lchrom = 4; // число генов
const int popsize = 6; //число популяции 
const double pmutation = 0.1; // вероятность мутации
const double pcross = 0.5;  // вероятность скрещивания
double min0; // максимальная приспособленность (близость к ответу )
int p1 = 0;
...
// поиск меньшего значени выживаемости у поколения
double statistics(individual* pop)
{
    ...
    for (int j = 1; j < popsize; j++)
    {
        ...
        if (pop[j].fitness < min0)
            min0 = pop[j].fitness; //  меньшее значение в популяции
    }
    sred = sred / popsize;
    return min0;
}
...
void select()
{
    ...
    {
        if (oldpop[j2].fitness < oldpop[j1].fitness) //выбираем менее приспособленную (турнир между парой) сильные- в начале, слабые в конце
        {
            intpop[j] = oldpop[j2];
...
void crossover(int par1, int par2, int ch1, int ch2)
{
    ...
    int l1 = rand() % (lchrom - 2) + 1; // случайная точка разрыва 1
    int l2 = rand() % (lchrom - 2) + 1; // случайная точка разрыва 2
    int raz1, raz2;
    if (l1 > l2)
    {
        raz1 = l2;
    ...
    if (flip(pcross)) // вероятность скрещивания 
    {
        ...
        for (int j = raz2; j < lchrom; j++) // гены от 2  точки разрыва
        {
            if (p = 1)
            {
                newpop[ch1].chrom[j] = mutation(oldpop[par1].chrom[j]);
...
void generation()
{
    ...
    cout << "Отсортированная популяция: " << endl << endl;
    ...
    cout << "Новая популяция:  " << endl << endl;
    for (int i = 0; i < popsize; i++) //печатаем первое поколение
    {
        cout << "Особь " << i + 1 << endl;
        for (int j = 0; j < maxstring; j++)
...
int main()
{
    ...
    int nmutation;
    int ncross;
    double t; //коэф минимальной выживаемости у поколения
    ...
    t = statistics(oldpop); // подсчет меньшего коэф выживаемости у первого поколения (min0 заполнены первым поколением)
      //массив для хранния минимальных значений
    double* minmass;
    ...
    cout << endl;
    cout << "Минимальное y(x) в поколении: " << minmass[0] << endl;
    cout << "среднее y(x) в поколении: " << sredmass[0] << endl;
    gen = 0;
    do
    {
        gen++;
        generation(); // создаем новое поколение 
        statistics(newpop); // находим меньший коэффициент выживаемости у поколения
        cout << min0 << endl;
        if (min0 < t)
            t = min0;
    ...
    ofstream Result("d:\\ResultV.txt"); //Созданием Файл  с результатом

    if (Result.is_open()) // проверка на открытие
        std::cout << "Ответ записан в файл ResultV!\n\n" << std::endl;
    else
    {
        std::cout << "Ошибка записи в файл" << std::endl;
        return -1;
    }
    Result << "Минимальное в поколении: " << endl;
